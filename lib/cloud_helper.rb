...
class CloudHelper
  ...
  def list_servers
    ...
    _format = "%12.11s %12.11s %17.16s %12.11s %10.10s %10.10s"
  ...
  def state_from_file(server)
    cmd = "cat #{StateFile}"
    results = server.ssh(cmd)
    if results && results.size > 0
      if results.first.status == 0
        return set_state(server,results.first.stdout.chomp)
      end
      return "error"
    end
    return "unkown"
  ...
  def state(server,state=nil)
    ...
    results = state
    if state
      results = set_state(server,state)
    else
      results = server.tags['cloud-state'] || state_from_file(server)
    end
    return results
  end
